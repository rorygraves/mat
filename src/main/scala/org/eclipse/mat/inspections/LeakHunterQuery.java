...
public class LeakHunterQuery implements IQuery {
    ...
    private FindLeaksQuery.SuspectsResultTable callFindLeaks(IProgressListener listener) throws Exception {
        return (FindLeaksQuery.SuspectsResultTable) SnapshotQuery.lookup("find_leaks", snapshot)
                .setArgument("threshold_percent", threshold_percent)
                .setArgument("max_paths", max_paths)
                .execute(listener);
    }
...
//                involvedClassloaders.add(accPointClassloader);
        ...
        try {
            IResult result = SnapshotQuery.lookup("path2gc", snapshot)
                    .setArgument("object", describedObject)
                    .execute(listener);
...
//                involvedClassLoaders.add(accPointClassloader);
    ...
    private Map<String, String> getTroubleTicketMapping(ITroubleTicketResolver resolver, List<IObject> classloaders)
        ...
        for (IObject suspect : classloaders) {
            ...
            if (ticket != null) {
                ...
                if (old != null) mapping.put(ticket, key + ", " + old);
            }
    ...
    private IResult getHistogramOfDominated(int objectId) throws SnapshotException {
        ...
        ListResult result = new ListResult(ClassHistogramRecord.class, suspects, "label",
                "numberOfObjects", "usedHeapSize", "retainedHeapSize") {
            @Override
        ...
        try {
            ...
            if (requestInfos != null && !requestInfos.isEmpty()) {
                ...
                for (CompositeResult.Entry requestInfo : requestInfos.getResultEntries())
                    builder.append(requestInfo.getName()).append(" ").append(
                            textResult.linkTo(Messages.LeakHunterQuery_RequestDetails,
                                    requestInfo.getResult())).append("<br>");
            ...
            if (stack != null) {
                ...
                builder.append("<p>");
                builder.append(Messages.LeakHunterQuery_StackTraceAvailable + " ").append(
